    ...
    public class FileNameTableDocument
    {
        ...
        private void ParseChainingSettingList(Expr e)
        {
            ...
            for (int i = 0; i < NumArg; ++i)
            {
                ...
                target.isLineHead = true;
                target.overrideOption(target);
                String prevFileName = target.filename;
                int optionChainCount = options.chains.Count;
                for (int j = 0; j < optionChainCount; ++j)
                {
                    ...
                    if (o.option == WaveOptions.OptionEnum.FILENAME)
                    {
                        if (String.IsNullOrEmpty(target.filename))
                        {
                            ...
                            target.filename += this.AutoGenPostfix;
                    ...
                    if (j + 1 >= optionChainCount)
                    {
                        Console.WriteLine("Gonna End, but target has no filename, prevFileName={0};autoGenPostFixNext={1}", prevFileName, target.autoGenPostFixNext);
                        if (!String.IsNullOrWhiteSpace(prevFileName) && !String.IsNullOrWhiteSpace(target.autoGenPostFixNext))
                        {
                            target.filename = prevFileName + target.autoGenPostFixNext;
                            WaveSplitOptionLines.Add(target);
                        }
